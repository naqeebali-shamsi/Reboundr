...
function JobsModule() {
  ...
  return (
    <>
      ...
      {
        userType === "jobseeker" ?
        <Search fetchCustomJob={fetchCustomJob} fetchSearchedJob={fetchSearchedJob} jobBeingSearched={jobBeingSearched} filteredJob={filteredJob}/>
        :
      ...
      {         
        ...
        .filter((j) => j.props.type === filteredJob.type && j.props.location === filteredJob.location) && (
          <Box><IconButton onClick={() => setFilteredJob(false)}>
          <CloseIcon />
          Custom Search
          </IconButton></Box>
        )
